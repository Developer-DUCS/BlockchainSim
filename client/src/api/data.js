...
router.post("/simulation", cors(), (req, res) => {
  const sim_id = 1;
  const email = "bryan_valencia_com";
  const sim_name = "My First Blockchain";
  const sim_shared = {
    emails: ["sean_lowry_com", "seth_workman_com"],
  };
  const sim_description = "The first blockchain simulation I ever created!";
  const sim_created = Date.now();
  const sim_modified = Date.now();
  const sim_blocks = {
  ...
  let qry =
    "INSERT INTO simulation VALUES (sim_id, email, sim_name, sim_shared, sim_description, sim_created, sim_modified, sim_blocks);";
  db.query(qry, (err, result) => {
    if (err) {
      console.log(err);
    } else {
